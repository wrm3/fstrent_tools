...
if "%BUMP_TYPE%"=="none" (
    ...
    for /f "usebackq tokens=3" %%i in (`type .bumpversion.cfg ^| findstr "current_version"`) do set OLD_VERSION=%%i
    ...
    for /f "usebackq tokens=3" %%i in (`type .bumpversion.cfg ^| findstr "current_version"`) do set NEW_VERSION=%%i
